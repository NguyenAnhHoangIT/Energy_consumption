...
def fetch_and_insert_data_range(last_fetched_time, db_connection):
    # Check if table exists, create if not
    check_and_create_table(db_connection)

    # Format the time in UTC as required by the API
    start_time = last_fetched_time.strftime('%Y-%m-%dT%H:%M:%SZ')
    end_time = (last_fetched_time + timedelta(days=60)).strftime('%Y-%m-%dT%H:%M:%SZ')
    # API URL with time filters
    api_url = f"https://api.gridstatus.io/v1/datasets/caiso_fuel_mix/query?api_key=734c5f6f8951487d90de7ef8ccb75832&start_time={start_time}&end_time={end_time}"
